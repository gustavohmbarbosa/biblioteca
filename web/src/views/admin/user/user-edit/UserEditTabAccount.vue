  ...
  <div id="user-edit-tab-info">
    ...
    <div class="vx-row">
      <div class="vx-col w-full">
        <div class="flex items-start flex-col sm:flex-row">
          <img v-if="data_local.image" :src="data_local.image" class="mr-8 rounded h-24 w-24" />
          <img v-else src="https://via.placeholder.com/300X300.png?text=SEM%20IMAGEM" class="mr-8 rounded h-24 w-24" />
          ...
          <div>
            ...
            <input type="file" class="hidden" ref="update_avatar_input" @change="update_avatar" accept="image/*">
            <vs-button class="mr-4 mb-4" @click="$refs.update_avatar_input.click()">Alterar Foto</vs-button>
            <!-- <vs-button type="border" class="mr-4" @click="$refs.update_avatar_input.click()">Change Avatar</vs-button> -->
            <vs-button type="border" color="danger" @click="data_local.image = null">Remove Image</vs-button>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="vx-row">
      <div class="vx-col md:w-1/2 w-full">
        <vs-input class="w-full mt-4" label="Nome"  v-validate="'required|alpha_spaces'" name="name" :value="data_local.name" />
        <!-- <span class="text-danger text-sm"  v-show="errors.has('name')">{{ errors.first('name') }}</span> -->

        <vs-input class="w-full mt-4" label="Email" type="email" v-validate="'required|email'" name="email" :value="data_local.email" />
    ...
    <div class="vx-row">
      <div class="vx-col w-full">
        <div class="mt-8 flex flex-wrap items-center justify-end">
          <vs-button class="ml-auto mt-2" @click="save_changes">Salvar Alterações</vs-button>
  ...
  methods: {
    ...
    update_avatar(input) {
      if (input.target.files && input.target.files[0]) {
        ...
        reader.onload = e => {
          this.data_local.image = e.target.result
        }
